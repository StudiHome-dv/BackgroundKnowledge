# Studio OS — Evidence-Based Studio Core Extraction v0.4
...
> Status: `Canonical Universal Sync — Reviewer Runtime Ready`  
> Version Change: `v0.3 → v0.4`는 승인된 Universal Consolidation의 Rule wording / Parent metadata / Runtime applicability sync만 반영한다.  
> Universal State: `5 Provisional + 1 Candidate / New Universal 0`  
> GSC State: `Active 0 / Retired Historical 3`  
> No-change: Genre Core Status / Scale Core Status / Market / Selection / Validation Status는 이번 version에서 변경하지 않는다.  
...
| UC-DESIGN-004 | **중요한 비통제 불확실성에는 의미 있는 Response Agency가 필요하다** | Universal | HIGH |
| UC-DESIGN-005 | **정보량보다 Decision-time Actionability와 Outcome-time Learnability를 평가한다** | Universal | HIGH |
...
문제는 Randomness 자체가 아니다.

```text
Important Uncertainty
+
No Preparation / Manipulation / Avoidance / Reserve / Recovery
↓
Player cannot meaningfully respond
```
...
- Slay the Spire
- Dicey Dungeons
- Strategy reserve / flexibility specialization

### Boundary

- **Deterministic Action Telegraph** — 공개된 Threat cue / response-window 문제는 `UC-DESIGN-005 + Action specialization`이 우선.
- **Known Deadline / Forecast** — 알려진 미래 압박은 자동으로 uncertainty가 아니다.
- **Intentional Chaos** — Chaos / surprise / 일부 horror에서 unpredictability 자체가 Product Promise일 수 있다.

### Parent Relationships

- `GC-DECK-003` — Availability / RNG Control specialization
- `GC-STRAT-009` — Reserve / Flexibility specialization
- Recovery Pattern 중 uncertainty response인 경우

### Candidate Metrics

**Structural / Machine**
- Uncertainty Source Count
- Response Option Availability
- Reroll / Avoid / Reserve / Recovery availability
...
> **정보 구조는 공개량이 아니라 ‘현재 의도한 판단을 수행할 수 있는가’와 ‘중요한 결과 이후 원인을 학습하고 다음 행동을 수정할 수 있는가’를 기준으로 평가한다.**

### Canonical Substructure

#### A. Decision-time Actionability

> 현재 플레이어가 의도된 판단을 수행하기 위해 필요한 State / Trend / Threat / Intent를 충분히 알 수 있는가?
...
> 중요한 결과 이후 플레이어가 왜 그 결과가 발생했는지 이해하고 다음 행동을 수정할 수 있는가?

관련:
- `GC-SIM-004`
- `GC-ACTION-003`
- Strategy Failure Attribution Pattern
- `GC-ROGUE-006` related
- Management diagnosis / failure-cause pattern

### Deduction Exception

다음은 UC005에 흡수하지 않는다.

- `GC-DEDUCT-001` — Inference Ownership
- `GC-DEDUCT-003` — Evidence Reachability
- `GC-DEDUCT-004` — Validation Leakage

`GC-DEDUCT-005`는 accessibility와 관련되지만 inference ownership replacement가 아니다.
...
**Player Telemetry**
- Info Check → Action Change
- Alert / Forecast Usage
...
Historical trace only. Active Canonical Title로 사용하지 않는다.
...
다음일 때만 로드:
- Progression이 핵심 Product Promise
- Build / Role / Capability Growth
- Meta Progression
- Vertical Power
...
UC006가 Candidate인 동안 Related Entry를 강제 Child로 재분류하지 않는다.
...
- Progression State / Unlock Definition
...
≠
Player-perceived Meaningful Growth
```

현재 Universal Provisional Core로 승격하지 않는다.
...
**Layer Ownership:** `RECLASSIFICATION CANDIDATE`  
**Potential Parent:** `Universal Validation Methodology`  
**Solo Residual:** `No Parallel Capacity / Opportunity Cost Amplification`

> `VAL-004`와 자동 Merge하지 않는다. `VAL-004`는 validation stage order, 이 Candidate Parent는 which risk to test first를 다룬다.
...
**Layer Ownership:** `RECLASSIFICATION CANDIDATE`  
**Potential Parent:** `Reference Analysis / Production Benchmarking Methodology`  
**Solo Residual:** `Limited-capacity amplification`

아직 새 Reference Analysis Namespace를 만들지 않는다.
...
| UC-DESIGN-006 Progression Should Match Its Intended Promise | **KEEP AS CANDIDATE** |
...
`Universal`은 `Always Applicable`을 의미하지 않는다.

## Universal Default Active

1. `UC-DESIGN-001` — Consequence Density
2. `UC-DESIGN-002` — Contextual Value
3. `UC-DESIGN-003` — Consequence → Next Decision
4. `UC-DESIGN-004` — Uncertainty Response Agency
5. `UC-DESIGN-005` — Actionable / Learnable Information

각 Core는 Applicability Gate 이후에만 Active Check한다.

## Universal Conditional Diagnostic

6. `UC-DESIGN-006 — Progression Should Match Its Intended Promise`

Progression이 핵심 Product Promise일 때만 `CONDITIONAL DIAGNOSTIC`으로 로드한다.

## Contextual Core 호출 방식

- 장르 → `GC-*`
- 제작규모 → `SC-*`
- Scale Handoff → `SCALE_HANDOFF-*`
- Optional GSC → Active `0`
- 지원 / 전시 → `SEL-*`
- Validation → `VAL-*`
...
Parent와 Child가 같은 Root Cause를 설명하면 Severity를 중복 부여하지 않는다.
...
## Canonical Runtime Snapshot — v0.4

```text
Universal Provisional: 5
Universal Candidate: 1
Universal Default: UC-DESIGN-001~005
Universal Conditional: UC-DESIGN-006
New Universal: 0
