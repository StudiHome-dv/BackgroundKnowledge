...
**Document:** `GENRE_CORE_MASTER_INDEX_v0.3`  
...
**Completed:** `Scale Core Consolidation / Genre × Scale Integration / Canonical GSC Consolidation / Universal Core Consolidation / Canonical Universal Source Sync / Reviewer Runtime v1.0 / Canonical Manifest v1.0 / Core Package v1.0 Assembly`  
**Universal Consolidation:** `COMPLETED / CANONICAL SYNCED`  
**Reviewer Runtime:** `v1.0 COMPLETE`  
**Canonical Manifest:** `v1.0 COMPLETE`  
**Core Package:** `v1.0 ASSEMBLED`  
**Current Release State:** `PACKAGE_READY_WITH_MIGRATION_FIX`  
**Next:** `Repository Migration / Package Release`  
**Pending:** `Repository Migration / Manifest Path Verification / Package Health Check / main verification / studio-os-v1.0 tag`
...
Universal이라는 이유로 모든 프로젝트에 PASS / FAIL을 강제하지 않는다.

- `UC-DESIGN-002` — Replay / systemic variation이 Product Promise가 아니면 CONDITIONAL / N/A 가능.
- `UC-DESIGN-004` — meaningful uncertainty가 없으면 N/A.
- `UC-DESIGN-006` — Progression이 핵심 Promise가 아니면 N/A.

## Universal Parent Registry

| ID | Status | Confidence | Router Role |
|---|---|---|---|
| `UC-DESIGN-001` — Consequence Density over Input Count | PROVISIONAL | VERY HIGH | consequence-bearing decision Parent |
| `UC-DESIGN-002` — Contextual Value Shift | PROVISIONAL | HIGH | repeated decision에서 context-dependent value Parent |
| `UC-DESIGN-003` — Consequence-to-Next-Decision Coupling | PROVISIONAL | HIGH | persistent state → future option / priority / risk → next decision Parent |
| `UC-DESIGN-004` — Uncertainty Requires Response Agency | PROVISIONAL | HIGH | important uncontrollable uncertainty → Preparation / Manipulation / Avoidance / Reserve / Recovery Parent |
| `UC-DESIGN-005` — Actionable Information | PROVISIONAL | HIGH | Decision-time Actionability + Outcome-time Learnability / Attribution Parent |
| `UC-DESIGN-006` — Progression Should Match Its Intended Promise | CANDIDATE | MEDIUM-HIGH | Progression Promise conditional diagnostic |

## Parent → Genre Specialization Map

| Parent | Phase / Mechanism | Related Genre Entries | Routing Rule |
|---|---|---|---|
| `UC001` | Consequence Density | `GC-ACTION-007`; `GC-MGMT-003` | Parent Root Issue 한 번; genre residual만 child |
| `UC002` | Contextual Value | `GC-DECK-001`; `GC-MGMT-006`; `GC-ROGUE-003`; `GC-NARR-009` related | `GC-NARR-001` 전체 흡수 금지 |
| `UC003` | Consequence → Next Decision | `GC-MGMT-004`; `GC-ROGUE-004`; `GC-STRAT-001`; `GC-STRAT-002`; `GC-SIM-002`; `GC-RPG-005`; `GC-NARR-003` partial | persistent state 자체가 아니라 future decision effect |
| `UC004` | Uncertainty Response | `GC-DECK-003`; `GC-STRAT-009`; uncertainty-response Recovery Pattern | deterministic threat cue는 UC005 우선 |
| `UC005` | Decision-time Actionability | `GC-MGMT-007`; `GC-STRAT-004`; `GC-ACTION-002` | 현재 판단을 수행할 정보 |
| `UC005` | Outcome-time Learnability / Attribution | `GC-SIM-004`; `GC-ACTION-003`; `GC-ROGUE-006` related; Strategy Failure Attribution Pattern | formal cause와 Human attribution 분리 |
| `UC006` | Progression Promise | `GC-RPG-001` specialization; `GC-DECK-012`; `GC-MGMT-008`; `GC-ROGUE-010` related | Candidate Parent; related entries 강제 child 금지 |

### Deduction Exception

`GC-DEDUCT-001/003/004`는 UC005에 환원하지 않는다.
...
`GC-DEDUCT-005`의 accessibility relation도 inference ownership replacement가 아니다.

## Universal Parent Coverage Matrix

| Universal Parent | Deck | Mgmt | Rogue | Deduct | Narr | Strat | Sim | RPG | Action |
|---|---|---|---|---|---|---|---|---|---|
| `UC001` | RELATED | SPECIALIZATION | RELATED | N/A | RELATED | RELATED | RELATED | RELATED | SPECIALIZATION |
| `UC002` | SPECIALIZATION | SPECIALIZATION | SPECIALIZATION | COUNTEREXAMPLE / N/A | SPECIALIZATION* | RELATED | RELATED | RELATED | CONDITIONAL |
| `UC003` | RELATED | SPECIALIZATION | SPECIALIZATION | N/A | SPECIALIZATION | SPECIALIZATION | SPECIALIZATION | SPECIALIZATION | RELATED |
| `UC004` | SPECIALIZATION | RELATED | RELATED | N/A | RELATED | SPECIALIZATION | RELATED | RELATED | CONDITIONAL |
| `UC005` | RELATED | SPECIALIZATION | RELATED | RELATED / EXCEPTION | SPECIALIZATION | SPECIALIZATION | SPECIALIZATION | RELATED | SPECIALIZATION |
| `UC006` | RELATED | RELATED | RELATED | N/A | RELATED | N/A | N/A | SPECIALIZATION | CONDITIONAL |

`* Narrative`: `GC-NARR-009`은 related specialization이지만 `GC-NARR-001` 전체는 independent Narrative residual.

Matrix는 Score가 아니다. Applicability Gate가 먼저다.

## Cross-Genre Pattern Registry

- **Failure Attribution** → `UC005 Outcome-time Learnability / Attribution`; 새 UC 없음.
- **Recovery as Continuing Decision** → `UC004 / UC003 / Genre Failure Structure`로 분기.
- **Identity / Differentiation** → underlying mechanism이 달라 Pattern만 유지.
- **Relevance-preserving Abstraction / Compression** → Strategy / Simulation strong; `SC-SOLO-002`와 혼합 금지.
...
| `GC-NARR-004` | MERGE CANDIDATE | `GC-NARR-001` | Branch convergence Parent 아래 판정 |
| `GC-NARR-009` | DUAL RELATION | `UC-DESIGN-002` related + `GC-NARR-001` merge/sub-rule candidate | Mechanical context와 Narrative meaning residual 분리 |
| `GC-SIM-009` | MERGE CANDIDATE | `GC-SIM-001` Sub-rule | Selective Simulation 아래 판정 |
| `GC-RPG-001` | UC006 CANDIDATE SPECIALIZATION | `UC-DESIGN-006` | Genre Candidate status unchanged |
| `GC-RPG-005` | UC003 SPECIALIZATION | `UC-DESIGN-003` | Character persistence residual만 child |
| `GC-ACTION-002` | UC005 DECISION-TIME SPECIALIZATION | `UC-DESIGN-005` | Threat cue / response-window residual |
| `GC-ACTION-003` | UC005 OUTCOME-TIME SPECIALIZATION + CROSS_GENRE_PATTERN | `UC-DESIGN-005` / Failure Attribution | **Genre Candidate status unchanged**; execution-cause residual 유지 |
| `GC-ACTION-007` | UC001 SPECIALIZATION | `UC-DESIGN-001` | Removed input + residual action agency |
| `GC-STRAT-004` | UC005 DECISION-TIME SPECIALIZATION | `UC-DESIGN-005` | Prediction / commitment residual |
| `GC-SIM-004` | UC005 OUTCOME-TIME SPECIALIZATION | `UC-DESIGN-005` | Causal diagnosis residual |
| `GC-DECK-003` | UC004 SPECIALIZATION | `UC-DESIGN-004` | Deck availability / RNG response만 child |

Genre Canonical Status / Confidence는 변경하지 않는다.
...
STEP 1  PROJECT_GENRE_ROUTING_PROFILE
STEP 2  Universal Applicability Gate — YES / CONDITIONAL / N/A
STEP 3  Applicable UC001~005 Parent Load
STEP 4  UC006 — Progression 핵심 Promise일 때만 CONDITIONAL DIAGNOSTIC
STEP 5  L3 Primary Genre Provisional
STEP 6  L3 High-confidence Candidate → WATCH
STEP 7  L2 Secondary Provisional
STEP 8  L2 Candidate → Risk Signal 있을 때만
STEP 9  L1 Supporting → Question 2~4개
STEP 10 Cross-Genre Parent / Child Deduplication
STEP 11 PROJECT_SCALE_ROUTING_PROFILE / Relevant Scale Core
STEP 12 Active Genre Scale Handoff
STEP 13 Optional GSC Registry Check — Active 0
STEP 14 Routing Specialization → Reviewer Action Hint
STEP 15 Market / Selection Layer when relevant
```
...
```text
AI State Access ≠ Player Knowledge
Machine Cause Trace ≠ Human Failure Attribution
Response Option Exists ≠ Human can perceive / execute it
Progression Unlock Exists ≠ Meaningful Growth
State Persists ≠ State changes Future Decisions
```
...
# 18. Double / Triple-Penalty Prevention Rules

1. **Parent-first** — Universal → Genre Specialization.
2. **One Root Issue** — 같은 Mechanism은 한 Root Issue로 집계.
3. **Applicability before Violation** — `N/A`는 PASS가 아니다.
4. **Child residual adds action, not duplicate severity.**
5. **Distinct Root Cause only** — Root Cause가 다를 때만 별도 Issue.
...
같은 Root Issue면 Severity 한 번.

## UC003 Management × Simulation Hybrid
...
를 자동으로 세 Issue로 만들지 않는다.

## Narrative Split

`GC-NARR-003`:
- State가 later decision/state를 바꾸지 않음 → `UC003`
- Later consequence의 cause를 연결할 수 없음 → `UC005`

Root Cause가 다를 때만 별도 Issue.
...
```text
Master Index Version: `v0.3`
Universal Baseline Version: `v0.4`
...
1. Scale Core Consolidation ✅
2. Genre × Scale Integration ✅
3. Canonical GSC Sync ✅
4. Universal Core Consolidation ✅
5. Canonical Universal Source Sync ✅
6. Reviewer Runtime v1.0 ✅
7. Canonical Manifest v1.0 ✅
8. Core Package v1.0 Assembly ✅

### Current Release State

`PACKAGE_READY`

Canonical Universal 상태:

```text
Universal Provisional: 5
Universal Candidate: 1
Default Active: UC001~005
Conditional: UC006
New Universal: 0
```
...
2. Manifest Path Verification
3. Package Health Check
4. merge to `main`
5. `studio-os-v1.0` tag
...
Status:
CANONICAL UNIVERSAL PARENT MAP SYNCED

Universal Provisional: 5
Universal Candidate: 1
Universal Default: UC001~005
Universal Conditional: UC006
New Universal: 0
...
Router Readiness:
REVIEWER_RUNTIME_v1.0_COMPLETE

Package Readiness:
PACKAGE_READY_WITH_MIGRATION_FIX

Primary Remaining Evidence Risk:
- RPG / Action Evidence Breadth
- Micro / Small Scale Evidence Breadth
- Failure Attribution Human Evidence
- Progression Promise / Vertical Power Boundary
```
...
UC001~005 Parent
↓
Conditional UC006 when applicable
↓
Genre Specialization
↓
Deduplication
